...
### Choosing Between netrc and Bearer Tokens

- **Use netrc** when: Your remote URI requires username/password authentication (e.g., private GitHub repositories, S3 with access keys)
- **Use bearer tokens** when: Your remote URI requires token-based authentication (e.g., OAuth2, API tokens, JWT tokens)

Both methods keep credentials out of your `runtime_env` URIs and prevent them from being logged or stored in plain text.
